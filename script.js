...
function form2Export(layoutName) {
    $(document).on('click', 'button[id="Form2Export' + layoutName + '"]', function () { //SET DATA Form2Export
        const str = document.getElementById("dataJson" + layoutName + "Area").value;
        const blob = new Blob([str], {
            type: 'text/plain'
        });
...
function textarea2ClipBoard(layoutName) {
    ...
    $(function () {
        $('#Textarea2ClipBoard' + layoutName).on('click', function () {
            ...
            $('#copy_alert' + layoutName).show().delay(2000).fadeOut(400); // アラート文の表示
        });
    });
};
...
function json2Table(targetJson, targetTable, layoutName) {
    $(function () {
        if ($('#' + targetTable + layoutName).length) { //すでに追加していたら削除する
            $('#' + targetTable + layoutName).empty();
        }
        let keys = Object.keys(targetJson[0]);
        $('#' + targetTable + layoutName).append('<table id="table_ele' + targetTable + layoutName + '"></table>'); //tableを追加する
        $('#table_ele' + targetTable + layoutName).append('<tr id="tr_ele' + targetTable + layoutName + '"></tr>'); /* 追加したtableにthを追加する */
        $('#tr_ele' + targetTable + layoutName).append('<th>' + '<div id="test" contenteditable="true">' + 'value' + '</div>' + '</th>');
        $('#tr_ele' + targetTable + layoutName).append('<th>' + '<div contenteditable="true">' + 'name' + '</div>' + '</th>');
        $('#tr_ele' + targetTable + layoutName).append('<th>' + '<div contenteditable="true">' + 'link' + '</div>' + '</th>');
        for (let i = 0; i < Object.keys(targetJson[0]).length; i++) { //targetJsonのデータをまるごと表示する
            $('#table_ele' + targetTable + layoutName).append('<tr id="tr_ele' + i + targetTable + layoutName + '"' + 'class="tr_ele_class' + targetTable + layoutName + '"' + '></tr>');
            $('#tr_ele' + i + targetTable + layoutName).append('<td>' + '<div id="tr_ele' + i + targetTable + layoutName + "_div" + '" contenteditable="true">' + targetJson[0][keys[i]][0].value + '</div>' + '</td>');
            $('#tr_ele' + i + targetTable + layoutName).append('<td id="td_ele_name' + i + targetTable + layoutName + '">' + '<div  contenteditable="true">' + keys[i] + '</div>' + '</td>');
            $('#tr_ele' + i + targetTable + layoutName).append('<td>' + '<div contenteditable="true">' + targetJson[0][keys[i]][0].link + '</div>' + '</td>');
...
function createHtml(elementId, layoutName) {
    $(function () {
        ...
        <button id="Textarea2ClipBoard' + layoutName + '">Textarea2ClipBoard</button>\
        <p id="copy_alert' + layoutName + '" style="display:none">Copied！</p> \
        \
        <!-- 01106 jsonの中身を部分的に表示する。。できた -->\
        <p id="rankingTable' + layoutName + '">rankingTable_Area</p> \
        \
        <!-- 01107 スプリットレイアウトのボタンを作る。。 -->\
        <button id="Split_Btn' + layoutName + '">Split</button>');
    });
};
...
function change2SaveJson(json, layoutName) {
    $(function () {
        for (let i = 0; i < json.length; i++) { //jsonのデータをまるごと表示する
